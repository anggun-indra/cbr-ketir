	...
	<div class="">
		<div class="box">
			<div class="box-header">
				<h3 class="box-title">CBR Cosine Similarity</h3>
			</div>

			<div class="box-body">
				<form action="{{route('cbrcos.store')}}" method="post">
					{{csrf_field()}}
				...
				<div class="box-body">
					...
					<table class="table table-responsive">
						...
						@foreach($arrCbrRes as $kas)
							<tr>
								<td>{{$i++}}</td>
								<td>{{$kas->fact1}} (w={{$kas->fact1w}}) => {{$kas->result1}}</td>
								<td>{{$kas->fact2}} (w={{$kas->fact2w}}) => {{$kas->result2}}</td>
								<td>{{$kas->fact3}} (w={{$kas->fact3w}}) => {{$kas->result3}}</td>
								<td>{{$kas->fact4}} (w={{$kas->fact4w}}) => {{$kas->result4}}</td>
								<td>{!! $kas->solving !!}</td>
								<td>
									{{$kas->result}}
								</td>
